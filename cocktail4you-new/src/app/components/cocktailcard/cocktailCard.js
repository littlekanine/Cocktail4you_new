		...
		<div>
			<div
				...
				style={{
					...
					borderRadius: '10px',
					borderTopLeftRadius: activeCard === cocktailId ? '10px' : '10px', // Coin supérieur gauche
					borderTopRightRadius: activeCard === cocktailId ? '10px' : '10px', // Coin supérieur droit
					borderBottomLeftRadius: activeCard === cocktailId ? '0px' : '10px', // Coin inférieur gauche (pas de border-radius)
					...
					<div className="flex">
						{connectUser ? (
							<Button
								...
								className={`button-star-svg transparent absolute zIndexPositive ${clickedStates[cocktailId] ? 'active' : ''}`}
							/>
						) : (
							<div></div>
						)}
			...
			<div className="flex info-container heightFull text-center scroll">
				{activeCard === cocktailId && (
					<div className={`extra-info flex center align-center widthFull padding10 column scroll ${isVisible ? 'visible' : ''}`}>
